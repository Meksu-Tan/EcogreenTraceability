        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(log, index) in transferStore.storageLogs" :key="log.id_trace_head" class="hover:bg-gray-50">
              ...
              <td v-if="!plantSelectionStore.selectedPlantId" class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{{ log.plant_code || '-' }}</td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button
                  v-if="log.to_trace_no && log.to_trace_no.toString().startsWith('7')"
                  @click="deactivateTransfer(log.id_trace_head, 'storage')"
                  :disabled="deactivatingId === log.id_trace_head"
                ...
                >
                  <i v-if="deactivatingId !== log.id_trace_head" class="fas fa-trash-alt"></i>
                  <i v-else class="fas fa-spinner fa-spin"></i>
                  {{ deactivatingId === log.id_trace_head ? 'Processing...' : 'Deactivate' }}
                </button>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(log, index) in transferStore.feedLogs" :key="log.id_trace_head" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button
                  v-if="log.to_trace_no && log.to_trace_no.toString().startsWith('7')"
                  @click="deactivateTransfer(log.id_trace_head, 'feed')"
                  :disabled="deactivatingId === log.id_trace_head"
                  class="inline-flex items-center gap-1 rounded bg-red-50 px-2 py-1 text-xs font-medium text-red-700 hover:bg-red-100 disabled:opacity-50"
                >
                  <i v-if="deactivatingId !== log.id_trace_head" class="fas fa-trash-alt"></i>
