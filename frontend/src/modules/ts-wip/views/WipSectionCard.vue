  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="px-4 py-3 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      <div class="flex items-center gap-2">
        <i :class="['fas', icon, 'text-xs', iconColorClass]"></i>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <template v-if="hasFeed">
          <button @click="$emit('entry-feed')" class="px-3 py-1.5 text-xs font-bold text-white bg-green-600 hover:bg-green-700 rounded-lg transition-all flex items-center gap-1 cursor-pointer">
            <i class="fas fa-plus"></i> Entry Feed
          </button>
          <button @click="$emit('view-feed-log')" class="px-3 py-1.5 text-xs font-bold text-gray-600 bg-white hover:bg-gray-50 border border-gray-300 rounded-lg transition-all flex items-center gap-1 cursor-pointer">
            <i class="fas fa-history"></i> Log Feed
          </button>
        </template>
        ...
        <template v-if="hasRundown">
          <button @click="$emit('entry-rundown')" class="px-3 py-1.5 text-xs font-bold text-white bg-green-600 hover:bg-green-700 rounded-lg transition-all flex items-center gap-1 cursor-pointer">
            <i class="fas fa-plus"></i> Entry Rundown
          </button>
          ...
          <button @click="$emit('view-rundown-log')" class="px-3 py-1.5 text-xs font-bold text-gray-600 bg-white hover:bg-gray-50 border border-gray-300 rounded-lg transition-all flex items-center gap-1 cursor-pointer">
            <i class="fas fa-history"></i> Log Rundown
          </button>
        </template>
      </div>
    </div>
...
const props = defineProps({
  ...
  icon: { type: String, default: 'fa-database' },
