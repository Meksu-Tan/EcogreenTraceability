...
class Blending extends Model
{
    ...
    static function get_newBlendingEntryNo($request){
        ...
        $db = DB::select('SELECT a.entryNo
                            FROM (SELECT b.trace_no + 1 AS entryNo
                                   ...
                                   LIMIT 1) a
                           UNION ALL
                          --  8yymmdd02rundown01 (LPAD(RIGHT(?, 2), 2, "0") takes last 2 digits of $idPlant - eob1 = 02)
                          SELECT CONCAT("8", DATE_FORMAT(CURDATE(), "%y%m%d"), LPAD(RIGHT(?, 2), 2, "0"), IF(a.id_rundown <> "-", a.id_rundown, "00"), "01") AS entryNo
                            FROM m_material a
                           WHERE a.status = 1
                             AND a.id_material = ?
                           LIMIT 1
    ...
    static function post_blendingEntry($user, $request){
        ...
        for ($z = 0; $z < $lenDatMatl; $z++){
            ...
            /* VARIABLE ADJUSTMENT */
            $out_qty = $qtySource;
            $entry_no = substr_replace($entryNo, '0', 9, 1); /* REPLACE RUNDOWN_ID TO FEED_ID */
