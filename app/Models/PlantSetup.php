...
class PlantSetup extends Model
{
    ...
    static function get_plant_dt(){
        ...
        $db = DB::select('SELECT a.id_plant, CONCAT(a.code_2, " | ", a.code_3) AS code, a.description, a.status,
                                 ...
                                 a.updated_by, a.id_tank, IFNULL(b.total_plant, 0) AS total_plant,
                                 a.description AS plant
                            FROM m_plant a
                            LEFT JOIN (SELECT COUNT(b.id_plant_tail) AS total_plant, b.id_plant
                                         FROM m_plant_detail b
                                        WHERE b.status = 1
                                        GROUP BY b.id_plant) b
