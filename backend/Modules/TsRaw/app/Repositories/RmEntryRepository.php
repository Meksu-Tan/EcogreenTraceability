...
class RmEntryRepository implements RmEntryRepositoryInterface
{
    ...
    public function saveRmEntry(array $data, string $user): array
    {
        ...
        try {
            ...
            $tankTailVal = !empty($data['id_tank_tail']) ? (is_array($data['id_tank_tail']) ? json_encode(array_values($data['id_tank_tail'])) : $data['id_tank_tail']) : null;
        ...
        try {
            ...
            foreach ($datTempMaterial as $row) {
                ...
                $feedParams = [
                    ...
                    'id_tank_tail' => $id_tankSourceNo_val,
