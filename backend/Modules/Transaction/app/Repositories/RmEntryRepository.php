...
use Modules\Transaction\Models\BalanceDetail;
use Modules\Transaction\Models\BalanceTemporary;
use Modules\Transaction\Models\TraceHeader;
use Modules\Transaction\Models\TraceDetail;
use Modules\Transaction\Models\MaterialDocument;
        ...
        $query = "SELECT
                            ...
                            CONCAT(' | ',
                                COALESCE(
                                    (
                                        ...
                                        FROM JSON_TABLE(
                                            IF(JSON_VALID(COALESCE(bh.id_sloc_tail, bh.id_tank_tail)), COALESCE(bh.id_sloc_tail, bh.id_tank_tail), '[]'),
                                            '$[*]' COLUMNS (sloc_id INT PATH '$')
                                        ) AS jt
                                        JOIN m_sloc_detail h ON h.id_sloc_tail = jt.sloc_id AND h.status = 1
                                    ),
                                    REPLACE(REPLACE(REPLACE(COALESCE(bh.id_sloc_tail, bh.id_tank_tail), '[', ''), ']', ''), '\"', '')
                                )
                            ),
                            ''
                        )
                    ...
                    FROM (
                        ...
                        WHERE status = 1
                          ...
                          AND (id_plant = ? OR ? = 0)
                          AND COALESCE(id_sloc, id_tank) IN ($inClause)
                        ORDER BY id_balance_head DESC
                    ) bh
                    INNER JOIN m_material m ON bh.id_material = m.id_material AND m.type = ?
                    LEFT JOIN m_sloc sl ON COALESCE(bh.id_sloc, bh.id_tank) = sl.id_sloc AND sl.status = 1
                    LEFT JOIN (
