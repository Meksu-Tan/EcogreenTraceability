...
class BalanceHeader extends Model
{
    ...
    {
        return $this->hasMany(\Modules\Transaction\Models\BalanceDetail::class, 'id_balance_head', 'id_balance_head')
                    ->where('status', 1);
    }

    public function traceHeaders()
    {
        return $this->hasMany(\Modules\Transaction\Models\TraceHeader::class, 'id_balance_head', 'id_balance_head');
    }
