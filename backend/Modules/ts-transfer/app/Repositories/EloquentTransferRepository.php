...
class EloquentTransferRepository implements TransferRepositoryInterface
{
    ...
    {
        // Consistent w/ ts-blending: t_balance_header.qty, prefix (1,2,7,8,9) per business-process.md §5
    ...
    public function getTransferList(int $plantId, int $page = 1, int $perPage = 5): array
    {
        ...
        $traceNoList = array_map(function ($row) {
            return $row->trace_no;
        }, $traceNoList);
        ...
        $selectSql = "a.entry_date, b.material_document,
            ...
            '.TraceHelper::fromPlantNameExpression('b.from_trace_no').' AS from_plant_name,
            '.TraceHelper::plantCodeExpression('a.trace_no')." AS plant_code_from_trace,
            ...
            CASE WHEN ABS(COALESCE(bs.qty,0) - a.qty) > 0.005 THEN {$fmtOnly3('COALESCE(bs.qty,0)')} ELSE {$fmtOnly3('a.qty')} END AS qty_supplier,
            ' >>> ' AS raw_sloc,
        ...
        $result = DB::connection($this->connection)->table('t_balance_header as a')
            ...
            ->leftJoin(DB::raw('(SELECT b.id_balance_head,
                                 ...
                                 MAX(CASE WHEN b.to_trace_no = (SELECT c.to_trace_no FROM t_trace_header c WHERE CAST(SUBSTRING(c.to_trace_no,1,1) AS INTEGER) = 7 AND '.TraceHelper::only14Digit('c.to_trace_no').' AND CAST(SUBSTRING(c.to_trace_no,9,1) AS INTEGER) <> 0 AND c.status = 1 ORDER BY c.to_trace_no DESC LIMIT 1) THEN 1 ELSE NULL END) AS is_last_row,
            ...
            ->leftJoin('m_plant as p', function ($join) {
                $join->on(DB::raw(TraceHelper::plantCodeExpression('a.trace_no')), '=', DB::raw('RIGHT(p.code_3, 2)'))->where('p.status', 1);
            })
    ...
    public function getUpdateSupplierMaterial(int $idMaterial, int $idSloc, int $plantId): ?object
    {
        ...
        $result = DB::connection($this->connection)->select(
            'SELECT CONCAT('.$dateFmtSql.', CAST(? AS TEXT), b.code_4, UPPER(a.code_matl_supplier)) AS supplierCode,
               ...
               LEFT JOIN (SELECT c.id_supplier FROM m_supplier c WHERE c.status = 1 AND c.type = CAST(? AS TEXT)) c ON 1=1
              LIMIT 1',
    ...
    public function postAdjEntrySupplier(string $user, string $adjNumber, int $idSupplier, int $idMaterial, float $qty, string $batchSap, string $plantId): array
    {
    ...
    {
        return app(TransferBalanceService::class)->createBalanceHeader($data);
    }
    ...
    {
        return app(TransferBalanceService::class)->createTraceHeader($data);
    }

    public function createTraceDetail(array $data): int
    {
        return DB::connection($this->connection)->table('t_trace_detail')->insertGetId($data, 'id_trace_tail');
    }

    public function cleanupTemporaryByEntryNo(string $entryNo): void
    {
        DB::connection($this->connection)->delete(
            'DELETE FROM t_balance_temporary WHERE entry_no = ?',
            [$entryNo]
        );
    }
    ...
    public function getSlocConflictList(): array
    {
        return DB::connection($this->connection)->select(
            "SELECT ta.id_balance_head, ta.trace_no, ta.source_sloc, ta.dest_sloc,
                    ta.qty, ta.entry_date, m.description AS material,
                    ps.description AS source_desc, pd.description AS dest_desc
               FROM t_transfer_approval ta
               LEFT JOIN t_balance_header bh ON ta.id_balance_head = bh.id_balance_head
               LEFT JOIN m_material m ON ta.id_material = m.id_material
               LEFT JOIN m_sloc ps ON ta.source_sloc::jsonb @> to_jsonb(ps.id_sloc)
               LEFT JOIN m_sloc pd ON ta.dest_sloc::jsonb @> to_jsonb(pd.id_sloc)
