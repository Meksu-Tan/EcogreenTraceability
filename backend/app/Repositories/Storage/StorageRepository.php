...
class StorageRepository implements StorageRepositoryInterface
{
    ...
    public function getAllTanks(): array
    {
        ...
        return DB::select('
            SELECT a.id_tank, CONCAT(a.code_2, " | ", a.code_3) AS code,
            ...
            FROM m_tank a
            LEFT JOIN (
                SELECT COUNT(b.id_tank_tail) AS total_tank, b.id_tank
                FROM m_tank_detail b WHERE b.status = 1 GROUP BY b.id_tank
            ) b ON a.id_tank = b.id_tank
    ...
    {
        $r = DB::select('SELECT * FROM m_tank WHERE id_tank = ?', [$id]);
        return $r[0] ?? null;
    }
    ...
    {
        $exists = DB::select('
            SELECT COUNT(id_tank) as cnt FROM m_tank
        ...
        $result = DB::insert('
            INSERT INTO m_tank (code_2, code_3, code_4, id_plant, description, created_by)
            VALUES (?, ?, ?, ?, ?, ?)
        ', [$data['code_2'], $data['code_3'], $data['code_4'] ?? null, $data['id_plant'], $data['description'], $data['created_by']]);
        if ($result) {
            $last = DB::select('SELECT id_tank FROM m_tank ORDER BY id_tank DESC LIMIT 1');
            DB::insert('INSERT INTO log_transactions (log_module, log_type, log_description, created_by) VALUES (?, ?, ?, ?)', [
                'M_STORAGE_TANK', 'ADD', 'ID: ' . $last[0]->id_tank . ' | CODE: ' . $data['code_3'] . ' / NAME: ' . $data['description'], $data['created_by'],
            ]);
        }
        return (bool) $result;
    }
    ...
    {
        $old = DB::select('SELECT code_2, code_3, description, id_plant FROM m_tank WHERE id_tank = ?', [$id]);
        ...
        $result = DB::update('
            UPDATE m_tank SET code_2 = ?, code_3 = ?, code_4 = ?, id_plant = ?, description = ?, updated_by = ?
             WHERE id_tank = ?
    ...
    public function deactivateTank(int $id, string $user): bool
    {
        ...
        return (bool) DB::update('UPDATE m_tank SET status = "0", updated_by = ? WHERE id_tank = ?', [$user, $id]);
    }
    ...
    public function activateTank(int $id, string $user): bool
    {
        ...
        return (bool) DB::update('UPDATE m_tank SET status = "1", updated_by = ? WHERE id_tank = ?', [$user, $id]);
    }
    ...
    {
        $r = DB::select('SELECT * FROM m_tank_detail WHERE id_tank_tail = ?', [$id]);
        return $r[0] ?? null;
    }

    public function createDetail(array $data): bool
    {
        $exists = DB::select('SELECT COUNT(id_tank_tail) as cnt FROM m_tank_detail WHERE tf_number = ? AND status = 1', [$data['tf_number']]);
        if ($exists[0]->cnt >= 1) return false;
        $result = DB::insert('INSERT INTO m_tank_detail (id_tank, tf_number, created_by) VALUES (?, ?, ?)', [
            $data['id_tank'], $data['tf_number'], $data['created_by'],
        ]);
        if ($result) {
            $last = DB::select('SELECT id_tank_tail FROM m_tank_detail ORDER BY id_tank_tail DESC LIMIT 1');
            DB::insert('INSERT INTO log_transactions (log_module, log_type, log_description, created_by) VALUES (?, ?, ?, ?)', [
                'M_STORAGE_TANK', 'ADD', 'ID: ' . $last[0]->id_tank_tail . ' | ID_TANK: ' . $data['id_tank'] . ' / TF_NUMBER: ' . $data['tf_number'], $data['created_by'],
            ]);
        }
        return (bool) $result;
    }
    ...
    {
        $exists = DB::select('SELECT COUNT(id_tank_tail) as cnt FROM m_tank_detail WHERE tf_number = ? AND status = 1 AND id_tank_tail != ?', [$data['tf_number'], $id]);
        if ($exists[0]->cnt >= 1) return false;
        $old = DB::select('SELECT tf_number FROM m_tank_detail WHERE id_tank_tail = ?', [$id]);
        if (empty($old)) return false;
        DB::insert('INSERT INTO log_transactions (log_module, log_type, log_description, created_by) VALUES (?, ?, ?, ?)', [
            'M_STORAGE_TANK', 'UPDATE',
        ...
        $result = DB::update('UPDATE m_tank_detail SET tf_number = ?, updated_by = ? WHERE id_tank_tail = ?', [
    ...
    public function deactivateDetail(int $id, string $user): bool
    {
        ...
        return (bool) DB::update('UPDATE m_tank_detail SET status = "0", updated_by = ? WHERE id_tank_tail = ?', [$user, $id]);
    }
    ...
    public function activateDetail(int $id, string $user): bool
    {
        ...
        return (bool) DB::update('UPDATE m_tank_detail SET status = "1", updated_by = ? WHERE id_tank_tail = ?', [$user, $id]);
    }
