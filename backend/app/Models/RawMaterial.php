...
class RawMaterial extends Model
{
    ...
    static function get_dtRmList($request){
        ...
        $db = DB::select('SELECT a.id_balance_head, a.id_material, a.id_tank, a.id_tank_tail, a.id_plant,
    ...
    static function deleteSupplier($id, $user, $request = null){
        $mode = $request ? $request->input('mode', 'ADD') : 'ADD';

        if ($mode === 'UPDATE') {
            DB::update('UPDATE t_balance_detail
                           SET status = 0, updated_by = ?
                         WHERE id_balance_tail = ?', [$user, $id]);
            DB::update('UPDATE t_trace_detail
                           SET status = 0, updated_by = ?
                         WHERE id_balance_tail = ?', [$user, $id]);

            $idHead = $request ? $request->input('id_balance_head') : null;
            if ($idHead) {
                $dat = DB::select('SELECT SUM(a.init_qty) AS qty
                                     FROM t_balance_detail a
                                    WHERE a.id_balance_head = ?
                                      AND a.status = 1', [$idHead]);
                $new_total_qty = $dat[0]->qty ?? 0;
    ...
    static function post_rmEntry($user, $request){
        ...
        if (is_array($id_tank_tail)) {
            $id_tank_tail = array_map('strval', $id_tank_tail);
        }
